---
title: "VoE Occlusion Exp1: pupil size analysis"
date: "22/08/2022"
output:
...
load(file = "voe_occlusion_exp1_workspace.RData")
```
...
sample.data <- read_parquet(file = "data/VoE_occlusion_Exp1_sample_report.parquet")%>%
...
*Plot raw data
```{r}
ggplot(data = sample.data %>% filter(condition != "hab"), aes(x = time.frame, y = RIGHT_PUPIL_SIZE)) +
  ylab("Pupil size") +
  xlab("Time (in ms)") +
  geom_point(aes(color = condition), alpha = 0.3, size = 0.5) +
  facet_wrap(~subject)+
    annotate("rect",
    xmin = 2000, xmax = 2700, ymin = -Inf, ymax = Inf,
    alpha = .2
  ) +
  xlim(0, 8000) +
    theme_bw()+
  ...
  theme(panel.grid.major.x = element_blank(),
        ...
        legend.title = element_blank(), legend.position = c(0.85, 0.8), legend.text=element_text(size=12))
```
...
artefact_check<-ggplot(data = sample.data%>% filter(condition != "hab"), aes(x = time.frame, y = RIGHT_PUPIL_SIZE_no_blinks)) +
  ylab("Pupil size") +
  xlab("Time (in ms)") +
  geom_point(aes(color = condition), alpha = 1, size = 0.5) +
  facet_wrap(~subject)+
  xlim(0, 8000) +
    theme_bw()+
  scale_color_manual(values=c("darkorange", "dodgerblue"))+
...
ggsave(artefact_check, filename = "graphs/voe_occlusion_exp1_artefact_check_100.png", width=10, height=8)
```
...
```{r eval=FALSE}
puphist <- ggplot(sample.data.no.artef, aes(x = RIGHT_PUPIL_SIZE_no_blinks)) + geom_histogram(aes(y = ..count..), 
...
exp.data.pupil.processed <- sample.data %>%
  ...
  select(subject, time.frame, condition, trial, RIGHT_GAZE_X_no_blinks, RIGHT_GAZE_Y_no_blinks, RIGHT_PUPIL_SIZE_no_blinks) %>%
  group_by(subject, condition, time.frame) %>%
...
exp.data.pupil.processed.downsampled <- exp.data.pupil.processed %>%  
  group_by(subject, condition, bin_low)%>%
...
```{r}
plot_interpolated_individual<-ggplot(data = exp.data.pupil.processed.downsampled%>% filter(condition != "hab"), aes(x = bin_low, y =pupil.base.corrected.binned )) +
  ylab("Pupil size") +
  xlab("Time (in ms)") +
...
ggsave(plot_interpolated_individual, filename = "graphs/voe_occlusion_exp1_interpolated_individual.png", width=10, height=8)
```
...
plot.interpolated_group <- ggplot(data = pupil.group.level %>% filter(condition != "hab"), aes(x = bin_low, y = mean.pupil.corrected.binned)) +
  ...
  geom_vline(aes(xintercept=baseline.end), lty=3)+
    xlim(0, 8000) +
    theme_bw()+
  scale_color_manual(values=c("darkorange", "dodgerblue"))+
  scale_fill_manual(values=c("darkorange", "dodgerblue"))+
  theme(panel.grid.major.x = element_blank(),
        ...
        panel.grid.major.y = element_blank(),
        panel.grid.minor.y = element_blank(),
        legend.title = element_blank(), legend.position = c(0.85, 0.85), legend.text=element_text(size=12),
        legend.key = element_blank(),
    legend.background = element_rect(fill = "transparent"))

plot.interpolated_group

ggsave(plot.interpolated_group, filename = "graphs/voe_occlusion_exp1_interpolated_group.png", height = 5, width = 6, scale = 0.6)
```

```{r}
save.image(file = "voe_occlusion_exp1_workspace.RData")
```
...
dat <- exp.data.pupil.processed.downsampled%>%
  filter(bin_low >= 1800 & bin_low <= 5800, condition!="hab")%>% #select interest period
...
ggsave("graphs/voe_occlusion_exp1_pupil_size_sup_info.png", width=11, height=11, scale=0.7)
...
plot_diff_m2<-ggplot(data=plot_diff_m2, aes(x=time, y=est))+
  ...
  theme_bw()+
  scale_x_continuous(name="Time (in ms)", breaks=c(1000, 2000, 3000, 4000, 5000))+ ylab("Est. difference in pupil size")+ggtitle("Disappear - Reappear")+
    theme(panel.grid.major.x = element_blank(),
        panel.grid.minor.x = element_blank(),
        panel.grid.major.y = element_blank(),
        panel.grid.minor.y = element_blank())+
...
ggsave(plot_diff_m2, filename = "graphs/VoE_occlusion_exp1_GAMM_diffcurve.png", width=6, height=5, scale=0.6)

```
* plotting partial effects
```{r}
pdf("graphs/VoE_occlusion_exp1_GAMM_partial_effect.pdf", width=10, heigh=8)
...
png("graphs/VoE_occlusion_exp1_GAMM_partical_effect2.png", width=25, height=20, units="cm", res=600)
...
* Summed effects
```{r}
png("graphs/voe_occlusion_exp1_m2_summed_effect.png", width=14, height=10, units="cm", res=600)
...
```{r}
ai.data <- read_csv("data/ET_VoE_occlusion_dynamicAOI.csv") %>%
...
ball.data <- xx %>%
  ...
  mutate(x_approx = na.approx(x_average, na.rm = FALSE, maxgap = 500), x_l_approx = na.approx(x_l), x_r_approx = na.approx(x_r))
...
gaze_occlusion_plot<-ggplot(data = gaze_plot_data, aes(x = time.frame, y = mean.x)) +
  ...
  geom_ribbon(data=gaze_plot_data %>% filter(condition == "hab"), aes(x = time.frame, ymin = 536, ymax = 1000), fill = "#E7CA0B", alpha = 0.75) +
     geom_ribbon(data=gaze_plot_data %>% filter(condition != "hab"), aes(x = time.frame, ymin = 528, ymax = 548), fill = "#E7CA0B", alpha = 0.75) +
    geom_ribbon(data = gaze_plot_data %>% filter(condition == "con") %>% filter(time.frame < 3252), aes(x = time.frame, ymin = x_l_approx, ymax = x_r_approx, fill = condition), alpha = 0.75) +
  geom_line(data = gaze_plot_data %>% filter(condition == "con") %>% filter(time.frame < 3252), aes(x = time.frame, y = x_approx), color = "#0A1585") +
  geom_ribbon(data = gaze_plot_data %>% filter(condition == "exp") %>% filter(time.frame < 1910), aes(x = time.frame, ymin = x_l_approx, ymax = x_r_approx), fill = "#0A1585", alpha = 0.5) +
  geom_line(data = gaze_plot_data %>% filter(condition == "exp") %>% filter(time.frame < 1910), aes(x = time.frame, y = x_approx), color = "#0A1585") +
  geom_ribbon(data = gaze_plot_data %>% filter(condition == "hab") %>% filter(time.frame < 1910), aes(x = time.frame, ymin = x_l_approx, ymax = x_r_approx), fill = "#0A1585", alpha = 0.5) +
  geom_line(data = gaze_plot_data %>% filter(condition == "hab") %>% filter(time.frame < 1910), aes(x = time.frame, y = x_approx), color = "#0A1585") +
    geom_point(aes(fill = x), alpha = 0.1, size=0.5) +
  geom_path(aes(y = median.x, fill = y), size = 1.2) +
   geom_ribbon(aes(ymin = mean.x - se.x, ymax = mean.x + se.x), alpha = 0.3)+
...
```{r}
library(cowplot)
occlusion_ini_photo <- "screenshots/occlusion_ini.jpg"

occlusion_exp_mid_photo <- "screenshots/occlusion_exp_mid.jpg"
occlusion_exp_fin_photo <- "screenshots/occlusion_exp_fin.jpg"
occlusion_con_mid_photo <- "screenshots/occlusion_con_mid.jpg"
occlusion_con_fin_photo <- "screenshots/occlusion_con_fin.jpg"

occlusion_ini <- ggdraw(theme(plot.margin = unit(c(0, 0, 0, 0), "cm"))) + draw_image(occlusion_ini_photo)
occlusion_exp_mid <- ggdraw(theme(plot.margin = unit(c(0, 0, 0,0), "cm"))) + draw_image(occlusion_exp_mid_photo)
occlusion_exp_fin <- ggdraw(theme(plot.margin = unit(c(0, 0, 0, 0), "cm"))) + draw_image(occlusion_exp_fin_photo)
occlusion_con_mid <- ggdraw(theme(plot.margin = unit(c(0, 0, 0, 0), "cm"))) + draw_image(occlusion_con_mid_photo)
occlusion_con_fin <- ggdraw(theme(plot.margin = unit(c(0, 0, 0, 0), "cm"))) + draw_image(occlusion_con_fin_photo)

occlusion_pics_exp<-plot_grid(occlusion_ini, occlusion_exp_mid, occlusion_exp_fin, ncol=3)
occlusion_pics_con<-plot_grid(occlusion_ini, occlusion_con_mid, occlusion_con_fin, ncol=3)
```
## Dwell time plot
```{r}
...
plot_dwell_end_IP_pole_IA <- ggplot(data = dwell.data, aes(x = condition, y= IA_DWELL_TIME)) +
  #Add geom_() objects
   geom_point(data = dwell.data %>% filter(condition =="Reappear"), aes(x = condition2), color = 'darkorange', size = 1.5, 
              alpha = .5) +
   geom_point(data = dwell.data %>% filter(condition =="Disappear"), aes(x = condition2), color = 'dodgerblue', size = 1.5, 
              alpha = .5) +
   geom_line(aes(x = condition2, group = subject), color = 'lightgray', alpha = .5) +
  
   geom_half_boxplot(
     data = dwell.data %>% filter(condition=="Reappear"), aes(x=condition2, y = IA_DWELL_TIME), position = position_nudge(x = -.4), 
     side = "r",outlier.shape = NA, center = TRUE, errorbar.draw = TRUE, width = .1, 
     fill = 'darkorange', alpha = .5) +
   
   geom_half_boxplot(
     data = dwell.data %>% filter(condition=="Disappear"), aes(x=condition2, y = IA_DWELL_TIME), position = position_nudge(x = .25), 
     side = "r",outlier.shape = NA, center = TRUE, errorbar.draw = TRUE, width = .1, 
...
ggsave(plot_dwell_end_IP_pole_IA, filename = "graphs/voe_occlusion_exp1_dwell_end_IP_pole_IA.png", height=5, width=5, scale=0.8)
```
...
pg_occlusion<-plot_grid(gaze_occlusion_plot, plot_dwell_end_IP_pole_IA,  ncol=2,nrow=1, rel_widths = c(2,1), labels = c("C","D") )

pg_occlusion1<-plot_grid(plot.interpolated_group, plot_diff_m2, rel_widths = c(2,1),labels=c("E", "F"))

ggsave(pg_occlusion1, filename = "graphs/VoE_occlusion__Exp1_pupil_size_pg1.png", width=10, height=4, scale=0.8)

pg.occlusion2<-plot_grid(pg_occlusion,pg_occlusion1, ncol=1)
pg.occlusion3<-plot_grid(occlusion_pics_con, occlusion_pics_exp, pg.occlusion2, ncol=1, rel_heights = c(0.5, 0.5, 1.5), labels = c("A", "B", ""))

pg.occlusion3

ggsave(pg.occlusion3, filename = "graphs/VoE_occlusion_exp1_figure.png", width = 10, height=13, scale=0.75)
ggsave(pg.occlusion3, filename = "graphs/VoE_occlusion_exp1_figure.pdf", width = 10, height=13, scale=0.75)
```
...
dynamic.aoi.data <- sample.data %>%
  filter(time.frame < 13501 & RIGHT_GAZE_X > 0 & RIGHT_GAZE_X < 1024 & RIGHT_GAZE_Y > 0 & RIGHT_GAZE_Y < 768) %>%
...
r2 <- dynamic.aoi.data %>%
  filter(time.frame < 1800 & time.frame > 500) %>% #ip until the ball disappeared
  droplevels() %>%
